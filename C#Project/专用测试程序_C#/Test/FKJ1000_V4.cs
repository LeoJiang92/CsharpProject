...
namespace MotorControlBoard
{
    public class FKJ1000_V4 : IFZJ
    {
        ...
        public FKJ1000_V4(int PortNum, byte devIndex, string scriptPath)
        {
            if (devIndex >= 100)
            {
        ...
        public FKJ1000_V4(UsbDeviceManager dev, byte devIndex, string scriptPath)
        {
        ...
        public override uint PutCardToICReader()
        {
            ...
            try
            {
                ...
                if ((this.cfgFunctionSwitch & 0x01) == 0x01)//欧姆龙出卡不会被阻挡，就不用做这些处理
                {
                    ...
                    this.MotoGoToPosition(10, this.cfgY_InCard, true, false);
                }
                else
                {
                ...
                this.ClearDevCondition();
                script = udM.LoadScriptFromFile("02_01_入卡到读卡器.txt");
                ...
                for (int a = 0; a <= 3; a++)
                {
                    ...
                    {
                        this.ClearDevCondition();
                        this.FKQRunScript(script, 0, 0, 0);//如果没有发卡成功，尝试继续发卡
                    }
        ...
        public override uint PutCardToTransferCar()
        {
            ...
            try
            {
                ...
                for (int i = 0; i <= 20; i++)
                {
                    ...
                    if (motor5Position != cfgX_InCard)  //没有到达指定位置
                        return ERR_CARD_WRONG_POSITION | 5;

                    MotoGetPosition(10, out motor10Position);
                    if (motor10Position != cfgY_InCard)//没有到达指定位置
                ...
                this.ClearDevCondition();
                script = udM.LoadScriptFromFile("02_02_入卡到中转机构.txt");
                r = this.FKQRunScript(script, 0, 0, 0);
                if (r != 0)
                {
                    this.ClearDevCondition();
        ...
        public override uint PutCardToTransferCarFromOutput()
        {
            ...
            try
            {
                ...
                {
                    this.ClearDevCondition();
                    string script = udM.LoadScriptFromFile("02_05_读卡器返卡到中转机构.txt");
                    r = this.FKQRunScript(script, 0, 0, 0);
                    if (r != 0)
                    {
                        this.ClearDevCondition();
                ...
                else if (this.CardIsInOutput())
                {
                    ...
                    string script = udM.LoadScriptFromFile("02_06_出卡口返卡到读卡器.txt");
                    r = this.FKQRunScript(script, 0, 0, 0);
                    if (r != 0)
                    {
                        this.ClearDevCondition();
                    ...
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile("02_05_读卡器返卡到中转机构.txt");
                    r = this.FKQRunScript(script, 0, 0, 0);
                    if (r != 0)
                    {
                        this.ClearDevCondition();
        ...
        public override uint GetCardToTransferCar(int x, int y)
        {
            uint r;
            string hexData,script;
            int changeFrequency1, changeFrequency2, changeFrequency3;
        ...
        public override uint GetCardToICReader()
        {
            ...
            try
            {
                ...
                this.ClearDevCondition();
                script = udM.LoadScriptFromFile("03_02_取卡到读卡器.txt");
                r = this.FKQRunScript(script, 0, 0, 0);
                if (r != 0)
                {
                    this.ClearDevCondition();
        ...
        public override uint GetCardToUser(bool isHoldCard, int step)
        {
            ...
            try
            {
                ...
                {
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile("03_03_取卡到用户.txt");
                ...
                {
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile("03_04_取卡到卡兜.txt");
                    r = FKQRunScript(script, 0, 0, 0);
                    if (r != 0)
                    {
                        this.ClearDevCondition();
