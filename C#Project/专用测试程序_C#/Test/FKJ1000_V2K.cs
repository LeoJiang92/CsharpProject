...
{
    public class FKJ1000_V2K : IFZJ
    {

        string InitScriptTxt = "";//发卡机初始化脚本
        string SenderToInReaderScriptTxt = "";//入卡到读卡器脚本
        string InReaderToTransferScriptTxt = "";//入卡到中转机构脚本
        string TransferUpScriptTxt = "";//中转机构抬起脚本
        string TransferToCardSlotScriptTxt = "";//入卡到卡槽脚本
        string TransferResetScriptTxt = "";//中转机构复位脚本
        string TransferLayFlatScriptTxt = "";//中转机构放平脚本 
        string CardSlotToTransferScriptTxt = "";//取卡到中转机构脚本
        string TransferToOutReaderScriptTxt = "";//取卡到出卡读卡器脚本
        string OutReaderToUserScriptTxt = "";//取卡到用户脚本
        string OuterToOutReaderScriptTxt = "";//出卡口返回读卡器脚本
        string OutReaderToTransferScriptTxt = "";//出卡读卡器返回中转机构脚本
        string ReturnToTransferScriptTxt = "";//卡片返回中转机构脚本
        string TranferToBadCardScriptTxt = "";//放卡到废卡存放区脚本
        string OutReaderToCardPocketScriptTxt = "";//出卡到面板卡兜
        ...
        public FKJ1000_V2K(int PortNum, byte devIndex, string scriptPath)
        {
            if (devIndex >= 100)
            {
        ...
        public FKJ1000_V2K(UsbDeviceManager dev, byte devIndex, string scriptPath)
        {
        ...
        {
            InitScriptTxt = "01_整机初始化.txt";//发卡机初始化脚本
            InReaderToTransferScriptTxt = "02_02_入卡到中转机构.txt";//入卡到中转机构脚本
            TransferToCardSlotScriptTxt = "02_03_入卡到卡槽.txt";//入卡到卡槽脚本  
            TransferUpScriptTxt = "04_02_抬起中转机构.txt";//中转机构抬起脚本
            TransferResetScriptTxt = "04_04_复位中转机构.txt";//中转机构放平脚本
            TransferLayFlatScriptTxt = "04_03_放平中转机构.txt";//中转机构放平脚本
            CardSlotToTransferScriptTxt = "03_01_取卡到中转机构.txt";//取卡到中转机构脚本
            TransferToOutReaderScriptTxt = "03_02_取卡到读卡器.txt";//取卡到出卡读卡器脚本
            OutReaderToUserScriptTxt = "03_03_取卡到用户.txt";//取卡到用户脚本
            OuterToOutReaderScriptTxt = "02_06_出卡口返卡到读卡器.txt";//出卡口返回读卡器脚本
            OutReaderToTransferScriptTxt = "02_05_读卡器返卡到中转机构.txt";//出卡读卡器返回中转机构脚本
            ReturnToTransferScriptTxt = "02_04_卡返回到中转机构.txt";//卡片返回中转机构脚本
            TranferToBadCardScriptTxt = "04_01_放卡到废卡槽.txt";//放卡到废卡存放区脚本
            OutReaderToCardPocketScriptTxt = "03_04_取卡到卡兜.txt";//出卡到面板卡兜

            if ((this.cfgFunctionSwitch & 0x04) == 0x0)//如果第3位为0，则该发证机为银行卡发证机
            {
                InitScriptTxt = "01_整机初始化(欧姆龙读卡器版).txt";
                SenderToInReaderScriptTxt = "02_01_入卡到读卡器(欧姆龙读卡器版).txt";
        ...
        public override bool CardIsInIcReaderOutput()
        {
            if ((this.cfgFunctionSwitch & 0x04) == 0x04)
            {
                if (this.SensorState(4).Equals("01"))
                    return true;
                return false;
            }
            else if ((this.cfgFunctionSwitch & 0x04) == 0x0)
        ...
        public override uint PutCardToICReader()
        {
            ...
            try
            {
                ...
                this.ClearDevCondition();
                string script = udM.LoadScriptFromFile(SenderToInReaderScriptTxt);
        ...
        public override uint PutCardToCardSlot(int x, int y)
        {
            ...
            try
            {
                ...
                script = udM.LoadScriptFromFile(TransferUpScriptTxt);//抬起中转机构脚本运行
                ...
                OutputLog("   进入PutCardToCardSlot_清出条件：R=" + r, false);
                script = udM.LoadScriptFromFile(TransferToCardSlotScriptTxt);//入卡到卡槽脚本运行
                r = this.FKQRunScript(script, x, y, 0);
                ...
                {
                    if (this.CardIsInTransfer() && this.SensorState(8).Equals("00"))//如果卡还在中转机构，没有往前推
                    {
                        System.Threading.Thread.Sleep(500);
                        ...
                        this.ClearDevCondition();
                        OutputLog("       进入PutCardToCardSlot_清出条件：R=" + r, false);
                        script = udM.LoadScriptFromFile(TransferToCardSlotScriptTxt);
                        r = this.FKQRunScript(script, x, y, 0);
                        OutputLog("       进入PutCardToCardSlot_重试执行(02_03_入卡到卡槽.txt)：R=" + r, false);
                    }
                }
                if (r != 0 || this.SensorState(8).Equals("01"))
                {
                    this.ClearDevCondition();
                    if (SensorState(8).Equals("01"))
                    {
                        //回收
                        script = udM.LoadScriptFromFile(ReturnToTransferScriptTxt);
                        r = this.FKQRunScript(script, x, y, 0);
                    ...
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile(TransferResetScriptTxt);
                    r = this.FKQRunScript(script, x, y, 0);
                    ...
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile(TransferUpScriptTxt);
                    r = this.FKQRunScript(script, x, y, 0);
                    OutputLog("   进入PutCardToCardSlot_执行(04_02_抬起中转机构.txt)：R=" + r, false);
                    if (r != 0)
                    {
                ...
                this.ClearDevCondition();
                script = udM.LoadScriptFromFile(TransferLayFlatScriptTxt);
                r = this.FKQRunScript(script, x, y, 0);
                if (r != 0)
                {
                    this.ClearDevCondition();
        ...
        public override uint PutCardToBadCardSlot(uint param)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    script = udM.LoadScriptFromFile(TranferToBadCardScriptTxt);
                    r = this.FKQRunScript(script, 0, 0, 0);
                    if (r == 0)
                    {
                        if (!CardIsInTransfer())
        ...
        public override uint GetCardToTransferCar(int x, int y)
        {
            ...
            try
            {
                ...
                this.ClearDevCondition();
                script = udM.LoadScriptFromFile(CardSlotToTransferScriptTxt);
                r = this.FKQRunScript(script, x, y, 0);
                this.FKQRunScript("1=S(0211 " + this.GetCardJDQ(x) + " 00)", x, y, 0);
        ...
        public override uint GetCardToUser(bool isHoldCard, int step)
        {
            ...
            try
            {
                ...
                {
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile(OutReaderToUserScriptTxt);//运行取卡到用户脚本（夹卡）
                ...
                {
                    this.ClearDevCondition();
                    script = udM.LoadScriptFromFile(OutReaderToCardPocketScriptTxt);//运行取卡到出卡兜脚本（不夹卡）
                    r = FKQRunScript(script, 0, 0, 0);
                    if (r != 0)
                    {
                        this.ClearDevCondition();
        ...
        /// 脚本参数替换函数
        /// </summary>
        /// <param name="script">需要替换的脚本</param>
        ...
        protected override string ReplaceParams(string script, int x, int y, int Params)
        {
            ...
            str = str.Replace("##FX##", this.GetFX());
            str = str.Replace("##ZX##", this.GetZX());
            str = str.Replace("##UP##", this.GetUp());
            str = str.Replace("##DOWN##", this.GetDown());
            return str;
        }
        ...
        {
            if ((this.cfgFunctionSwitch & 0x20) == 0x20)//后排卡槽
            {
                return "00";
            }
            else if ((this.cfgFunctionSwitch & 0x20) == 0x0)//前排卡槽
            {
                return "01";
            }
            return "";
        ...
        public override string GetCardJDQ(int x)
        {
            if ((this.cfgFunctionSwitch & 0x20) == 0x20)//后排卡槽
            {
                return ((x + 5).ToString().PadLeft(2, '0'));
            }
            else if ((this.cfgFunctionSwitch & 0x20) == 0x0)//前排卡槽
            {
